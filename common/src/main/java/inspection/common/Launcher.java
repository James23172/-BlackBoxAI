...
public class Launcher {
    ...
    private static boolean autoCompile() {
        // 检查是否已经编译过（common/target/classes 存在且 pom.xml 未更新）
        File commonClasses = new File(PROJECT_ROOT, "common\\target\\classes");
        File pomFile = new File(PROJECT_ROOT, "pom.xml");
        if (commonClasses.exists() && pomFile.lastModified() < commonClasses.lastModified()) {
            System.out.println("📦 已有编译产物，跳过编译");
            return true;
        }
